...
class ITCHParser:
    ...
    def _handle_add(self, body: bytes) -> None:
        ...
        order_ref, side_byte, shares, stock_bytes, price_itch = \
            struct.unpack('>QcI8sI', body[11:])

        price_ticks = price_itch // 100        # 500100 → 5001 cents
        side        = Side.BUY if side_byte == b'B' else Side.SELL

        # sanity checks — if these fail, the format string or byte layout is wrong
        assert 1 < price_ticks < 1_000_000,  f"implausible price: {price_ticks}"
        assert shares > 0,                   f"zero/negative shares: {shares}"
        assert order_ref not in self.book.orders, \
            f"duplicate Add for order_ref {order_ref} — already in book"
    ...
    def _handle_execute(self, body: bytes) -> None:
        ...
        """
        order_ref, executed_shares, match_number = struct.unpack('>QIQ', body[11:])
...
# ------------------------------------------------------------------ #
# Self-test against the synthetic file                                #
# ------------------------------------------------------------------ #
...
if __name__ == '__main__':
    ...
    print("\n--- sanity checks ---")
    assert book.bids == {5001: __import__('collections').deque(
        [o for o in book.orders.values()])}, ""
